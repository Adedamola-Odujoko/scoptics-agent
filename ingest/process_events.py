...
def setup_database(engine):
    ...
    with engine.connect() as connection:
        connection.execute(text("DROP TABLE IF EXISTS possession_spells;"))
        connection.execute(text("DROP TABLE IF EXISTS passes;"))
        connection.execute(text("DROP TABLE IF EXISTS shots;"))
        connection.execute(text("DROP TABLE IF EXISTS big_chances;"))
        connection.execute(text("""CREATE TABLE possession_spells (match_id TEXT, spell_id TEXT PRIMARY KEY, period INT, start_frame INT, end_frame INT, duration_seconds FLOAT, possessing_team TEXT, possessing_player_id TEXT);"""))
        connection.execute(text("""CREATE TABLE passes (pass_id TEXT PRIMARY KEY, match_id TEXT, period INT, frame INT, passer_id TEXT, receiver_id TEXT, passer_team TEXT, distance_m FLOAT, outcome TEXT);"""))
        connection.execute(text("""CREATE TABLE shots (shot_id TEXT PRIMARY KEY, match_id TEXT, period INT, frame INT, game_minute INT, game_second INT, shooter_id TEXT, shooter_team TEXT, start_x FLOAT, start_y FLOAT, outcome TEXT, xg FLOAT);"""))
        connection.execute(text("""CREATE TABLE big_chances (big_chance_id TEXT PRIMARY KEY, match_id TEXT, period INT, frame INT, player_id TEXT, team_name TEXT, x FLOAT, y FLOAT, num_defenders_between INT);"""))
        connection.commit()
...
def calculate_possession_spells(df_tracking, ball_id, engine):
    ...
    df_spells = df_possession.groupby('spell_group_id').agg(
        match_id=('match_id', 'first'), period=('period', 'first'), start_frame=('frame', 'min'), end_frame=('frame', 'max'),
        start_time=('timestamp', 'min'), end_time=('timestamp', 'max'),
        possessing_team=('team_name', 'first'), possessing_player_id=('trackable_object', 'first')
    ).reset_index(drop=True)
    if df_spells.empty:
        print("  -> No possession spells found.")
        return pd.DataFrame()
    df_spells['duration_seconds'] = (df_spells['end_time'] - df_spells['start_time']).dt.total_seconds()
    ...
    if not df_final_spells.empty:
        ...
        df_final_spells[['match_id', 'spell_id', 'period', 'start_frame', 'end_frame', 'duration_seconds', 'possessing_team', 'possessing_player_id']].to_sql('possession_spells', engine, if_exists='append', index=False)
...
def identify_passes(df_spells, df_tracking, ball_id, engine):
    ...
    if df_spells.empty or len(df_spells) < 2:
        ...
        return
    df_spells = df_spells.sort_values(['possessing_team', 'start_frame'])
    df_spells['receiver_id'] = df_spells['possessing_player_id'].shift(-1)
    df_spells['next_start_frame'] = df_spells['start_frame'].shift(-1)
    df_spells['next_start_time'] = df_spells['start_time'].shift(-1)
    df_spells['is_same_team_pass'] = df_spells['possessing_team'] == df_spells['possessing_team'].shift(-1)
    df_passes = df_spells[(df_spells['is_same_team_pass']) & (df_spells['possessing_player_id'] != df_spells['receiver_id'])].copy()
    time_diff = (df_passes['next_start_time'] - df_passes['end_time']).dt.total_seconds()
    df_passes = df_passes[(time_diff > 0.05) & (time_diff < 3.0)]
    if not df_passes.empty:
        df_ball_pos = df_tracking[df_tracking['trackable_object'] == ball_id][['frame', 'x', 'y']].set_index('frame')
        df_passes = df_passes.join(df_ball_pos, on='end_frame', rsuffix='_pass_start').join(df_ball_pos, on='next_start_frame', rsuffix='_pass_end')
        df_passes.dropna(subset=['x', 'y', 'x_pass_end', 'y_pass_end'], inplace=True)
        df_passes['distance_m'] = np.sqrt((df_passes['x_pass_end'] - df_passes['x'])**2 + (df_passes['y_pass_end'] - df_passes['y'])**2)
        df_passes['pass_id'] = df_passes.apply(lambda row: f"pass-{row['match_id']}-{row['end_frame']}", axis=1)
        df_to_insert = pd.DataFrame({'pass_id': df_passes['pass_id'], 'match_id': df_passes['match_id'], 'period': df_passes['period'], 'frame': df_passes['end_frame'], 'passer_id': df_passes['possessing_player_id'], 'receiver_id': df_passes['receiver_id'], 'passer_team': df_passes['possessing_team'], 'distance_m': df_passes['distance_m'], 'outcome': 'Completed'})
        df_to_insert.to_sql('passes', engine, if_exists='append', index=False)
        print(f"  -> Saved {len(df_to_insert)} passes.")
    else: print("  -> No passes found.")
...
def identify_big_chances(df_spells, df_players, home_team_name, pitch_length, engine):
    ...
    for _, spell in df_spells.iterrows():
        ...
        if defenders_between <= 2:
            big_chances.append({'big_chance_id': f"bc-{spell['match_id']}-{spell['end_frame']}", 'match_id': spell['match_id'], 'period': spell['period'], 'frame': spell['end_frame'], 'player_id': spell['possessing_player_id'], 'team_name': spell['possessing_team'], 'x': player_pos['x'], 'y': player_pos['y'], 'num_defenders_between': defenders_between})
    if big_chances:
    ...
    else: print("  -> No big chances identified.")
...
def identify_shots_and_calculate_xg(df_tracking, ball_id, home_team_name, pitch_length, match_id_str, engine):
    ...
    for i, shot in df_final_shots.iterrows():
        ...
        shots_for_db.append({'shot_id': shot_id, 'match_id': match_id_str, 'period': period, 'frame': frame, 'game_minute': game_minute, 'game_second': game_second, 'shooter_id': shot['shooter_id'], 'shooter_team': shot['shooter_team'], 'start_x': shot['player_x'], 'start_y': shot['player_y'], 'outcome': shot['outcome'], 'xg': xg})
    if shots_for_db:
